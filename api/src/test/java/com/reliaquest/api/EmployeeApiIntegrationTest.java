...
class EmployeeApiIntegrationTest {
        ...
        for (int attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt < maxRetries) {
                    ...
                    System.out.println("Operation " + operationName + " failed (attempt " + (attempt + 1) + "/"
                            + (maxRetries + 1) + "), retrying in " + delayMs + "ms. Error: " + e.getMessage());
    ...
    void testDeleteEmployee_IntegrationTest() {
        ...
        ResponseEntity<Employee> createResponse = retryOperation(
                () -> restTemplate.postForEntity(getBaseUrl(), createInput, Employee.class), "Create Employee", 3);
        ...
        ResponseEntity<Employee> getResponse = retryOperation(
                () -> restTemplate.getForEntity(getBaseUrl() + "/" + employeeId, Employee.class), "Get Employee", 3);
        assertEquals(HttpStatus.OK, getResponse.getStatusCode());

        // Step 3: Delete the employee with retry logic
        ResponseEntity<String> deleteResponse = retryOperation(
                () -> restTemplate.exchange(
                        getBaseUrl() + "/" + employeeId, HttpMethod.DELETE, HttpEntity.EMPTY, String.class),
                "Delete Employee",
                3);
